<div class="container-fluid com_news">	
	<section class="col-md-6 space c_get">
			<h2 class="section-title wow flipInX"><?php echo $wl_rmmuc_options['subscriber_form_title'];?></h2>			
			<?php if ($wl_rmmuc_options['subscriber_form_icon'] == null) { ?><span class="wow rubberBand"> <span class="<?php echo $wl_rmmuc_options['subscriber_form_icon'];?>  icon"></span> </span>
			<?php } else { ?>
			<span class="wow rubberBand">..........   <span class="<?php echo $wl_rmmuc_options['subscriber_form_icon'];?>  icon"></span>   ..........</span>
			<?php } ?>
			<h3 class="section_sub_title" data-sr="enter top"><?php echo $wl_rmmuc_options['subscriber_form_sub_title'];?></h3>
		...
		<?php if ($wl_rmmuc_options['subscribe_select']=='wp_mail'){ ?>	
					...
					<div class="subscribe-message">	
						...
						if(isset($_GET['act_code']) && $_GET['email']){
							...
							if(count($user_search_result)) {
								...
								<input type="text" name="f_name"  class="form-control wow slideInUp" placeholder="<?php echo $wl_rmmuc_options['sub_form_button_f_name'];?>" required="required">
								</div>
								<div class="col-md-6 form-group">
									<input type="text" name="l_name"  class="form-control wow slideInUp " placeholder="<?php echo $wl_rmmuc_options['sub_form_button_l_name'];?>" required="required">
								</div>
								<div class="col-md-12 form-group">
									<input type="mail" name="subscribe_email" id="edmm-sub-email2" class="form-control wow slideInUp subscribe-input-layout1 s2email" placeholder="<?php echo $wl_rmmuc_options['sub_form_subscribe_title'];?>" required="required">
									<span class="sub_error_msg" style="display:none; color:red;"><span style="color:red; font-size: 14px;"><?php _e("* ","weblizar"); ?></span><?php _e("Invalid email address.","weblizar"); ?></span>
								</div>
						...
						<div class="col-md-12 sub_btn form-group">					
							<button name="submit_subscriber" class="btn wow slideInUp subscriber_submit sub-submit" type="submit"><?php echo $wl_rmmuc_options['sub_form_button_text'];?></button>
						</div>
					</div>
				</form>
				<?php } ?>
			...
			<form method="post" action="" onsubmit="return validateForm2()" class="php_subscriber-form" name="php_subscriber-form">
				...
				<div class="form-group" data-sr="enter bottom over 1s and move 110px wait 0.5s">					
					...
					<div class="col-md-12 form-group">
						<input type="mail" name="subscribe_email" id="edmm-sub-email2" class="form-control wow slideInUp subscribe-input-layout1 s2email" placeholder="<?php echo $wl_rmmuc_options['sub_form_subscribe_title'];?>" required="required">
						<span class="sub1_error_msg" style="display:none; color:red;"><span style="color:red; font-size: 14px;"><?php _e("* ","weblizar"); ?></span><?php _e("Invalid email address.","weblizar"); ?></span>
					</div> 
					<div class="col-md-12">					
						<button name="php_submit_subscriber" class="btn wow slideInUp subscriber_submit" type="submit"><?php echo $wl_rmmuc_options['sub_form_button_text'];?></button>
					</div>	
				</div>
			</form>
		<?php } ?>	
