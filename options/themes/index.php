...
<head>
  ...
  <meta name="keywords" content="<?php echo $wl_rmmuc_options['page_meta_keywords']; ?>">
  <meta name="description" content="<?php echo $wl_rmmuc_options['page_meta_discription']; ?>">
 <?php if($wl_rmmuc_options['search_robots'] == 'on') { ?>
 <meta name="robots" content="<?php echo $wl_rmmuc_options['rmmuc_robots_meta'];?>" /> 
<?php } ?>
...
<?php }elseif($template_bg_select == 'Custom_Background'){ ?>
...
<div id="main_div_container" class="container">
	<div class="carousel-caption">
		<h4 class="logo_class"><?php $site_logo_value = $wl_rmmuc_options['site_logo'];
		if ($site_logo_value == 'logo_text')
		{echo $wl_rmmuc_options['logo_text_value'];}
			elseif ( $wl_rmmuc_options['upload_image_logo'] == null) {
					echo $wl_rmmuc_options['logo_text_value'];
					}
			else{ ?>
				<img class="wow slideInUp" width="<?php echo $wl_rmmuc_options['logo_width'];?>" height="<?php echo $wl_rmmuc_options['logo_height'];?>" src="<?php echo esc_url($wl_rmmuc_options['upload_image_logo']); ?>">
			</h4> <?php }?>
					
	<?php $layout_status = $wl_rmmuc_options['layout_status'];
		if ($layout_status == 'under_construction_switch')
			{ ?>
				<h1 class="wow slideInUp"><?php echo $wl_rmmuc_options['under-construction_title'];?></h1>
				<h4 class="wow zoomInUp"><?php echo $wl_rmmuc_options['under-construction_sub_title'];?></h4>
				<h3 data-sr="enter top"> <?php echo $wl_rmmuc_options['under-construction_message'];?></h3>
				<?php include 'counterclock-progressbar.php'; ?>				
					<?php	$button_onoff = $wl_rmmuc_options['button_onoff'];
					if ($button_onoff == 'on')
						{ if($wl_rmmuc_options['button_text']) {?>	
						<a class="btn wow bounceInUp" href="<?php echo $wl_rmmuc_options['button_text_link'];?>" data-sr="enter bottom over 1s and move 110px wait 0.3s"><?php echo $wl_rmmuc_options['button_text'];?></a>
						<?php  } } ?>
				</div>
	<?php   }
		elseif ($layout_status == 'maintenance_switch')
			{ ?>
				<h1 class="wow slideInUp"><?php echo $wl_rmmuc_options['maintenance_title'];?></h1>
				<h2 class="wow zoomInUp"><?php echo $wl_rmmuc_options['maintenance_sub_title'];?></h2>
				<h4 data-sr="enter top"><?php echo $wl_rmmuc_options['maintenance_message'];?></h4>
				<?php include 'counterclock-progressbar.php'; ?>				
					<?php	$button_onoff = $wl_rmmuc_options['button_onoff'];
					if ($button_onoff == 'on'){
						if($wl_rmmuc_options['button_text']) {?>	
...
?>
	...
	<footer>
		<div class="container">
			<div class="col-md-8 col-sm-8 footer_copy">
					<?php echo $wl_rmmuc_options['footer_copyright_text'];?> <a href="<?php echo $wl_rmmuc_options['footer_link']; ?>"> <?php echo $wl_rmmuc_options['footer_link_text'];?> </a>
			</div>
			<div class="col-md-4 col-sm-4 footer_social">
				<ul class="social animated fadeInDownBig">
					<?php for($i=1; $i<=$wl_rmmuc_options['total_Social_links']; $i++){
						...
						?>
						<li class="team-social-<?php echo $social_class; ?>"><a target="<?php if($wl_rmmuc_options['link_tab_'.$i]=='on') echo '_blank'; ?>" href="<?php echo $wl_rmmuc_options['social_link_'.$i]; ?> <?php echo $wl_rmmuc_options['social_link_'.$i]; ?>"><i class="<?php echo $wl_rmmuc_options['social_icon_'.$i]; ?> icon"></i></a></li>
							<?php }
						} ?>
				</ul>
			</div>
	...
	<!-- Fooetr -->
	<a href="#" class="back-to-top"><i class="fa fa-angle-left"> Scroll Up</i></a>
</div>
...
if($wl_rmmuc_options['link_admin']=='on') { 
	if (is_user_logged_in() ) {
			...
			if($UserData->roles[0] == 'administrator') {
				...
				?>
				<a class="btn left_side_link" target="blank" href="<?php echo get_admin_url(); ?>" data-sr="enter bottom over 1s and move 110px wait 0.3s"> <?php echo __($admin_link_text,'weblizar');?> </a>
